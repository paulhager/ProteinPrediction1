...
learningRates = [1e-6, 5e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3]
#learningRates = [5e-4]

if optimize:
  bestMCC = 0
  for batchSize in range(1000, 499, -500):
    for hiddenNodes in range(500, 19, -80):
      for weightNonbinding in range(9, 0, -1):
        weightNonbinding = weightNonbinding/10
        for weightBinding in range(9, 0, -1):
          weightBinding = weightBinding/10
          for learning_rate in learningRates:
            for epochs in range(420, 19, -60):
              for momentum in range(9, 0, -1):
                ...
                if mcc > bestMCC:
                  ...
                  bestFN = fn
                  bestCutoff = cutoff
                  f = open('/home/h/hagerp/ProteinPrediction/ProteinPrediction1/BestParams.txt', 'w')
