...
sp <- ggplot(epochMCCDT, aes(x=Epoch)) + geom_smooth(aes(y=MCC, colour = "MCC"), show.legend = FALSE) 
sp
...
sp2 <- ggplot(epochMCCDT, aes(x=Epoch)) + geom_smooth(aes(y=Cutoff, colour = "Cutoff"), show.legend = FALSE)
sp2
...
library(gridExtra)
library(dplyr) 
paramGridSearchRaw <- read.csv('allParams_both.csv')
