...
def trainParamOptimizerEdit(batchSize, hiddenNodes, weightNonbinding, weightBinding, learning_rate, epochs, momentum, train_data, train_labels, test_data, test_labels):
  D_in, H, D_out = 40, hiddenNodes, 1
  trainTensors = torch.tensor(train_data, dtype=torch.float)
  labelTensors = torch.tensor(train_labels, dtype=torch.float)
  train_and_labels = TensorDataset(trainTensors, labelTensors)
  ...
  optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, momentum=momentum)
  fout = open("epochMCC.csv", "w+")
  fout.write("Epoch,MCC,Cutoff\n")
  testDataTensors = torch.tensor(test_data, dtype=torch.float)
  ...
  for t in range(epochs):
    ...
    if t % 5 == 0:
      ...
      if bestMCC == -1:
        bestMCC = 0
      fout.write(str(t)+","+str(bestMCC)+","+str(bestCutoff)+"\n")
  fout.close()
...
if optimize:
  ...
  cutoff, tp, fp, tn, fn, mcc = trainParamOptimizerEdit(batchSize, hiddenNodes, weightNonbinding, weightBinding, learning_rate, epochs, momentum, train_data, train_labels, test_data, test_labels)
  print("Best cutoff: "+str(cutoff))
  print("TP: "+str(tp))
  print("FP: "+str(fp))
  print("TN: "+str(tn))
